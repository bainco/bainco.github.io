...
<a class="nu-button" href="/course-files/projects/project01_template.zip" target="_blank">
...
{:.caption}
Created by Rachel Smith (former CS 110 student)

## Part 1: Create a make_creature function
In a file named `creature.py`, create a custom function called `make_creature` that makes a creature of your choosing. Feel free to use/modify/improve upon the function you already made in [HW3](hw3) (but remember, as described in [Tutorial 6](tutorial06), you'll need to make sure you can tag your creature correctly). It should have at least 4 parameters that enable the calling function to specify arguments for:
...
> **Important**: in order for the shapes that constitute your creature to “belong” together, each shape has to be tagged. When modifying your HW 3 creature, strongly consider replacing your various `make_shape` functions with their counterparts in `utilities.py` (those functions are setup to create the correct tags). DO NOT DO THIS BY JUST CHANGING THE FUNCTION NAMES. Make sure to look at the definitions of those functions in `utilities.py` to make sure that a) you know the function's name and b) you know what inputs it requires (positional arguments) and what names it gives its optional inputs. Replace each function call one at a time. if you used some of the advanced `canvas.create_...` functions, there are `make_...` versions available in `utilities.py` but keep in mind that you'll have to use them differently.
...
| 6 | 3 points | Periodically add or remove creatures and/or landscape objects to/from your scene. |
...
| 12 | 3 points | Make your creature animate with different versions like Mario in [Tutorial 5](tutorial05). |

{: .blockquote-no-margin}
> **IMPORTANT:**
> At the top of your `main.py` file please include a short paragraph (bullet points are fine) as a comment that explains which animation features you chose to implement in your terrarium. **IF WE DO NOT SEE THIS PARAGRAPH YOU MAY NOT RECEIVE POINTS FOR THE ANIMATION WORK YOU IMPLEMENTED.**
...
## Reference Materials
### Code Samples
There are lots of samples/demos of how to include user interaction in our [Lesson 16/17](../lectures/week06-lecture03) lecture files. There are also some advanced animation techniques in there for you to use as a reference.
...
* Use someone else's code from any other CS 110 course past or present. You will receive a 0 on the Project and be held in violation of our academic honestly policy.

## What to Turn In
1. Your code as a zip file to the Project 1 CODE Canvas assignment. This should include all of the files that make your code work.
1. MAKE SURE TO INCLUDE THE PARAGRAPH THAT EXPLAINS WHICH ANIMATION OPTIONS YOU IMPLEMENTED AS DESCRIBED in Part 4 above.
1. Your video as a `MP4` file (which is what Zoom creates) to the Project 1 VIDEO Canvas assignment.
